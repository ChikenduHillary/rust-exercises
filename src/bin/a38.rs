...
fn main() {
    ...
    let msg_one = msg_one.join().expect("failed to join mgs_one");
    let msg_two = msg_two.join().expect("failed to join mgs_two");
    let msg_three = msg_three.join().expect("failed to join mgs_three");
